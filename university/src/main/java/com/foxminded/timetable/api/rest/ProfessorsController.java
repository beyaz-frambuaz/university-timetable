...
import com.foxminded.timetable.api.*;
...
public class ProfessorsController implements ProfessorsApi {
    ...
    public CollectionModel<EntityModel<Professor>> findAll() {
        ...
        return new CollectionModel<>(models,
                linkTo(methodOn(ProfessorsApi.class).findAll()).withSelfRel());
    }
    ...
    public CollectionModel<EntityModel<Course>> findProfessorCourses(long id) {
        ...
        return new CollectionModel<>(models,
                linkTo(methodOn(ProfessorsApi.class).findById(
                        id)).withSelfRel());
    }

    @Override
    public CollectionModel<EntityModel<Student>> findCourseAttendees(long id,
            long courseId) {
        ...
        return new CollectionModel<>(models,
                linkTo(methodOn(ProfessorsApi.class).findCourseAttendees(id,
                        courseId)).withSelfRel(),
                linkTo(methodOn(ProfessorsApi.class).findById(id)).withRel(
                        "professor"),
                linkTo(methodOn(CoursesApi.class).findById(courseId)).withRel(
                        "course"));
    }
