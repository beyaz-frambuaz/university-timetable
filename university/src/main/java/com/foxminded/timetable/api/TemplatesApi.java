...
public interface TemplatesApi {
    ...
    @Operation(summary = "List all templates", tags = { "templates" },
               description = "Lists schedule templates that represent "
                       + "two week study cycle")
    @ApiResponse(responseCode = "200", description = "operation successful",
                 content = @Content(array = @ArraySchema(schema = @Schema(
                         implementation = ScheduleTemplate.class))))
    CollectionModel<EntityModel<ScheduleTemplate>> findAll();

    @GetMapping(path = "/{id}", produces = { "application/json" })
    @Operation(summary = "Find template by ID", tags = { "templates" },
               description = "Get single schedule template that is used to "
                       + "generate by-weekly schedule items")
