...
public interface SchedulesApi {
    ...
    @Operation(summary = "Date schedules", tags = { "schedules" },
               description = "Look up schedule items by semester date "
                       + "(optionally filter by either professor, group, "
    ...
    CollectionModel<EntityModel<Schedule>> findByDate(
            ...
            @RequestParam(name = "professorId", required = false) @Min(
                    1) Long professorId,
            @RequestParam(name = "groupId", required = false) @Min(
                    1) Long groupId,
            @RequestParam(name = "courseId", required = false) @Min(
                    1) Long courseId,
            @RequestParam(name = "auditoriumId", required = false) @Min(
                    1) Long auditoriumId);
    ...
    @ApiResponse(responseCode = "400", description = "invalid data submitted",
                 content = @Content(
                         schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<Schedule>> findByWeek(
    ...
    @Operation(summary = "Month schedules", tags = { "schedules" },
               description = "Look up schedule items by month "
                       + "(optionally filter by either professor, group, "
                       + "course, or auditorium")
    @ApiResponse(responseCode = "200", description = "operation successful",
                 content = @Content(array = @ArraySchema(
    ...
    @ApiResponse(responseCode = "400", description = "invalid data submitted",
                 content = @Content(
                         schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<Schedule>> findByMonth(
    ...
    @Operation(summary = "Reschedule", tags = { "schedules" },
               description = "Use to make one-time changes (substitute "
                       + "professor, move to a different auditorium, etc.) or"
                       + " reschedule all class occurrences. "
                       + "Saves edited schedule item. "
                       + "Optionally reschedules recurring class which "
                       + "involves editing existing schedules and underlying "
                       + "template (all further schedule items will be "
    ...
    @ApiResponse(responseCode = "404", description = "schedule not found",
                 content = @Content(
                         schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<Schedule>> reschedule(
            @PathVariable @Min(1) long id,
            @RequestBody @Valid Schedule rescheduled,
            @RequestParam(name = "recurring",
                          defaultValue = "false") boolean recurring);

    @GetMapping(path = "/{id}/available/auditoriums",
                produces = { "application/json" })
    @Operation(summary = "Find available auditoriums",
               tags = { "schedules", "auditoriums" },
    ...
    @ApiResponse(responseCode = "200", description = "operation successful",
                 content = @Content(array = @ArraySchema(
                         schema = @Schema(implementation = Auditorium.class))))
    @ApiResponse(responseCode = "400", description = "invalid ID submitted",
                 content = @Content(
                         schema = @Schema(implementation = ApiException.class)))
    @ApiResponse(responseCode = "404", description = "schedule not found",
                 content = @Content(
                         schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<Auditorium>> findAvailableAuditoriums(
            @PathVariable @Min(1) long id);

    @GetMapping(path = "/{id}/available/professors",
                produces = { "application/json" })
    @Operation(summary = "Find available professors",
               tags = { "schedules", "professors" },
    ...
    @ApiResponse(responseCode = "404", description = "schedule not found",
                 content = @Content(
                         schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<Professor>> findAvailableProfessors(
            @PathVariable @Min(1) long id);
    ...
    @ApiResponse(responseCode = "200", description = "operation successful",
                 content = @Content(array = @ArraySchema(schema = @Schema(
                         implementation = ReschedulingOption.class))))
    @ApiResponse(responseCode = "400", description = "invalid data submitted",
                 content = @Content(
                         schema = @Schema(implementation = ApiException.class)))
    @ApiResponse(responseCode = "404", description = "schedule not found",
                 content = @Content(
                         schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<ReschedulingOption>> findOptions(
            @PathVariable @Min(1) long id,
