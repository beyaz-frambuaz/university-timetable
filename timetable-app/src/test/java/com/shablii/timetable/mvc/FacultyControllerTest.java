...
class FacultyControllerTest {
    ...
    @Test
    public void getListShouldRequestProfessorsFromServiceAndAddToModel() throws Exception {

        List<Professor> professors = Collections.emptyList();
        given(timetableFacade.getProfessors()).willReturn(professors);

        mvc.perform(get("/timetable/faculty/list")).andExpect(model().attribute("professors", professors));
    ...
    public void getListShouldAddFlashMassagesToModel() throws Exception {
        ...
        mvc.perform(get("/timetable/faculty/list")).andExpect(model().attributeExists("errorAlert", "sessionExpired"));
    }

    @Test
    public void postListShouldRequestProfessorFromServiceCheckPresenceAddToSessionAndRedirectToHome() throws Exception {
        ...
        ResultActions resultActions = mvc.perform(
                post("/timetable/faculty/list").param("professorId", String.valueOf(id)));
        resultActions.andExpect(request().sessionAttribute("professor", professor))
    ...
    @Test
    public void postListShouldRedirectToListIfProfessorNotPresent() throws Exception {
    ...
    public void postListShouldValidateIdAndRedirectToListWithErrorMessageIfInvalid() throws Exception {
        ...
        long id = 0;
        MvcResult mvcResult = mvc.perform(post("/timetable/faculty/list").param("professorId", String.valueOf(id)))
                .andExpect(status().is3xxRedirection())
                .andExpect(flash().attributeExists("errorAlert"))
                .andExpect(redirectedUrl("/timetable/faculty/list"))
                .andReturn();
    ...
    public void getHomeShouldRequestTodaysScheduleFromFormatterAndReturnHomeView() throws Exception {
        ...
        mvc.perform(get("/timetable/faculty/professor/home").sessionAttr("professor", professor))
                .andExpect(status().isOk())
    ...
    public void getTwoWeekShouldRequestScheduleFromFormatterAndReturnTwoWeekView() throws Exception {
        ...
        mvc.perform(get("/timetable/faculty/professor/two_week").sessionAttr("professor", professor))
                .andExpect(status().isOk())
    ...
    public void postScheduleShouldRequestAndDisplayDaySchedulePerFormRequest() throws Exception {
        ...
        RequestBuilder requestBuilder = post("/timetable/faculty/professor/schedule").sessionAttr("professor",
                professor).flashAttr("scheduleForm", scheduleForm);
    ...
    @Test
    public void getCoursesShouldRequestAndDisplayProfessorCoursesWithAttendees() throws Exception {
        ...
        given(timetableFacade.getCourseAttendees(any(Course.class), any(Professor.class))).willReturn(courseAttendees);
        Map<Course, List<Student>> allCourseAttendees = new HashMap<>();
        allCourseAttendees.put(course, courseAttendees);

        mvc.perform(get("/timetable/faculty/professor/courses").sessionAttr("professor", professor))
                .andExpect(status().isOk())
    ...
    @ParameterizedTest
    @ValueSource(strings = { "/timetable/faculty/professor/home", "/timetable/faculty/professor/two_week",
            "/timetable/faculty/professor/courses" })
    public void getProfessorPagesShouldThrowExceptionIfNoProfessorInSession(String uri) throws Exception {
    ...
    @Test
    public void postScheduleShouldThrowExceptionIfNoProfessorInSession() throws Exception {
        ...
        MvcResult mvcResult = mvc.perform(
                post("/timetable/faculty/professor/schedule").flashAttr("scheduleForm", scheduleForm))
                .andExpect(status().is3xxRedirection())
                .andExpect(flash().attributeExists("sessionExpired"))
                .andExpect(redirectedUrl("/timetable/faculty/list"))
                .andReturn();
