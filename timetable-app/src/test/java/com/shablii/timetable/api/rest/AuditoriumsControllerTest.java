...
class AuditoriumsControllerTest {
    ...
    void findAllShouldRequestFromService() throws Exception {
        ...
        given(timetableFacade.getAuditoriums()).willReturn(Collections.singletonList(auditorium));

        mvc.perform(get(baseUrl).accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.auditoriumList[0].id").value(auditorium.getId()))
                .andExpect(jsonPath("$._embedded.auditoriumList[0].name").value(auditorium.getName()))
    ...
    void addNewShouldSaveAndReturnCreated() throws Exception {
        ...
        given(timetableFacade.saveAuditorium(any(Auditorium.class))).willReturn(auditorium);
    ...
    void findByIdShouldRequestFromService() throws Exception {
        ...
        mvc.perform(get(baseUrl + auditorium.getId()).accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(auditorium.getId()))
                .andExpect(jsonPath("$.name").value(auditorium.getName()))
                .andExpect(jsonPath("$._links.self").isNotEmpty())
    ...
    void editByIdShouldCallServiceToSave() throws Exception {
        ...
        mvc.perform(put(baseUrl + auditorium.getId()).contentType(MediaType.APPLICATION_JSON).content(payload))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(auditorium.getId()))
                .andExpect(jsonPath("$.name").value(auditorium.getName()))
                .andExpect(jsonPath("$._links.self").isNotEmpty())
    ...
    void deleteByIdShouldReturnNotImplemented() throws Exception {
        ...
        mvc.perform(delete(baseUrl + auditorium.getId())).andExpect(status().isNotImplemented());
    }
