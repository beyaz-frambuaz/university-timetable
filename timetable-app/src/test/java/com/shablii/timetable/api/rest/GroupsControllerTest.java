...
class GroupsControllerTest {
    ...
    void findAllShouldRequestFromService() throws Exception {
        ...
        given(timetableFacade.getGroups()).willReturn(Collections.singletonList(group));

        mvc.perform(get(baseUrl).accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$._embedded.groupList[0].id").value(group.getId()))
                .andExpect(jsonPath("$._embedded.groupList[0].name").value(group.getName()))
                .andExpect(jsonPath("$._embedded.groupList[0]._links").isNotEmpty())
    ...
    void findByIdShouldRequestFromService() throws Exception {
        ...
        mvc.perform(get(baseUrl + group.getId()).accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(group.getId()))
                .andExpect(jsonPath("$.name").value(group.getName()))
                .andExpect(jsonPath("$._links.self").isNotEmpty())
    ...
    void editByIdShouldCallServiceToSave() throws Exception {
        ...
        mvc.perform(put(baseUrl + group.getId()).contentType(MediaType.APPLICATION_JSON).content(payload))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(group.getId()))
                .andExpect(jsonPath("$.name").value(group.getName()))
                .andExpect(jsonPath("$._links.self").isNotEmpty())
    ...
    void deleteByIdShouldReturnNotImplemented() throws Exception {
        ...
        mvc.perform(delete(baseUrl + group.getId())).andExpect(status().isNotImplemented());
    }
