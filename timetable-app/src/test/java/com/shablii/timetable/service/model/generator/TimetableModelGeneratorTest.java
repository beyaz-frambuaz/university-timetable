...
class TimetableModelGeneratorTest {
    ...
    public void repositoryShouldHaveFiveReschedulingOptionsPerPeriod() {
        ...
        List<Long> optionsPerDayPerPeriod = timetableFacade.getOptions()
                .stream()
                .collect(groupingBy(ReschedulingOption::getDay, groupingBy(ReschedulingOption::getPeriod, counting())))
                .values()
                .stream()
    ...
    public void repositoryShouldHaveReschedulingOptionsForWorkDaysOnly() {
        ...
        assertThat(actual).noneMatch(
                option -> option.getDay() == DayOfWeek.SATURDAY || option.getDay() == DayOfWeek.SUNDAY);
    }
    ...
    public void scheduleTemplatesShouldContainAllCoursesForEachGroup() {
        ...
        Map<Group, List<Course>> eachGroupCourses = timetableFacade.getTwoWeekSchedule()
                .stream()
                .collect(groupingBy(ScheduleTemplate::getGroup, mapping(ScheduleTemplate::getCourse, toList())));
    ...
    public void scheduleTemplatesShouldContainAllProfessors() {
        ...
        assertThat(actual).usingElementComparatorIgnoringFields("courses").hasSameElementsAs(expected);
    }
    ...
    public void scheduleTemplatesShouldHaveNoAuditoriumCollisions() {
        ...
        List<DayOfWeek> workDays = Arrays.asList(DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY,
            ...
            for (DayOfWeek day : workDays) {
                for (Period period : Period.values()) {
                    ...
                    assertThat(templates).filteredOn(
                            template -> template.getWeekParity() == weekParity && template.getDay() == day
