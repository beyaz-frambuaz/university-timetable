...
public interface StudentsApi {
    ...
    ResponseEntity<EntityModel<Student>> addNew(@RequestBody @Valid Student student);
    ...
    @Operation(summary = "Edit student with ID", tags = { "students" },
               description = "Either updates existing or saves new " + "student, returns result")
    @ApiResponse(responseCode = "200", description = "operation successful",
                 content = @Content(schema = @Schema(implementation = Student.class)))
    @ApiResponse(responseCode = "400", description = "invalid data submitted",
                 content = @Content(schema = @Schema(implementation = ApiException.class)))
    EntityModel<Student> editById(@PathVariable @Min(1) long id, @RequestBody @Valid Student newStudent);
