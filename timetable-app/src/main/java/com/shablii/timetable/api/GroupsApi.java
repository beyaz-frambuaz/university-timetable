...
public interface GroupsApi {
    ...
    @Operation(summary = "Edit group with ID", tags = { "groups" },
               description = "Either updates existing or saves new " + "group, returns result")
    @ApiResponse(responseCode = "200", description = "operation successful",
                 content = @Content(schema = @Schema(implementation = Group.class)))
    @ApiResponse(responseCode = "400", description = "invalid data submitted",
                 content = @Content(schema = @Schema(implementation = ApiException.class)))
    EntityModel<Group> editById(@PathVariable @Min(1) long id, @RequestBody @Valid Group newGroup);
