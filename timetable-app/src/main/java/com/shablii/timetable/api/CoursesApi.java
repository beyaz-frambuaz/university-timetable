...
public interface CoursesApi {
    ...
    ResponseEntity<EntityModel<Course>> addNew(@RequestBody @Valid Course course);
    ...
    @Operation(summary = "Edit course with ID", tags = { "courses" },
               description = "Either updates existing or saves new " + "course, returns result")
    @ApiResponse(responseCode = "200", description = "operation successful",
                 content = @Content(schema = @Schema(implementation = Course.class)))
    @ApiResponse(responseCode = "400", description = "invalid data submitted",
                 content = @Content(schema = @Schema(implementation = ApiException.class)))
    EntityModel<Course> editById(@PathVariable @Min(1) long id, @RequestBody @Valid Course newCourse);
    ...
    @Operation(summary = "Find professors teaching course", tags = { "courses", "professors" })
    @ApiResponse(responseCode = "200", description = "found",
    ...
    @ApiResponse(responseCode = "404", description = "course not found",
                 content = @Content(schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<Professor>> findCourseProfessors(@PathVariable @Min(1) long id);
