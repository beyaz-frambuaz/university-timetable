...
import com.shablii.timetable.api.rest.assemblers.GroupModelAssembler;
...
public class GroupsController implements GroupsApi {
    ...
    public CollectionModel<EntityModel<Group>> findAll() {
        ...
        return new CollectionModel<>(models, linkTo(methodOn(GroupsApi.class).findAll()).withSelfRel());
    }

    @Override
    public ResponseEntity<EntityModel<Group>> addNew(Group group) {

        EntityModel<Group> entityModel = assembler.toModel(timetableFacade.saveGroup(group));
    ...
    public EntityModel<Group> findById(long id) {
        ...
        Group group = timetableFacade.getGroup(id)
                .orElseThrow(() -> new NotFoundException("Group with ID " + id + " could not be found"));
