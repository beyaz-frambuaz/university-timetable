...
public interface ProfessorsApi {
    ...
    ResponseEntity<EntityModel<Professor>> addNew(@RequestBody @Valid Professor professor);
    ...
    @Operation(summary = "Edit professor with ID", tags = { "professors" },
               description = "Either updates existing or saves new " + "professor, returns result")
    @ApiResponse(responseCode = "200", description = "operation successful",
                 content = @Content(schema = @Schema(implementation = Professor.class)))
    @ApiResponse(responseCode = "400", description = "invalid data submitted",
                 content = @Content(schema = @Schema(implementation = ApiException.class)))
    EntityModel<Professor> editById(@PathVariable @Min(1) long id, @RequestBody @Valid Professor newProfessor);
    ...
    @Operation(summary = "Find professor's courses", tags = { "professors", "courses" })
    @ApiResponse(responseCode = "200", description = "found",
    ...
    @ApiResponse(responseCode = "404", description = "professor not found",
                 content = @Content(schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<Course>> findProfessorCourses(@PathVariable @Min(1) long id);

    @GetMapping(path = "/{id}/courses/{courseId}/students", produces = { "application/json" })
    @Operation(summary = "Find students attending professor's course", tags = { "professors", "courses", "students" })
    @ApiResponse(responseCode = "200", description = "found",
                 content = @Content(array = @ArraySchema(schema = @Schema(implementation = Student.class))))
    @ApiResponse(responseCode = "400", description = "invalid professor or course ID submitted",
                 content = @Content(schema = @Schema(implementation = ApiException.class)))
    @ApiResponse(responseCode = "404", description = "professor or course not found",
                 content = @Content(schema = @Schema(implementation = ApiException.class)))
    CollectionModel<EntityModel<Student>> findCourseAttendees(@PathVariable @Min(1) long id,
