...
public class ManagementUniversityGroupsController {
    ...
    @GetMapping("/")
    public String groups(Model model, @ModelAttribute("successAlert") String successAlert,
        ...
        Map<Group, List<Student>> groupedStudents = timetableFacade.getGroupedStudents();
    ...
    public String groupSchedule(Model model, @ModelAttribute @Valid ScheduleForm scheduleForm,
    ...
    public String rename(RedirectAttributes redirectAttributes, @ModelAttribute @Valid RenameForm renameForm) {
        ...
        Optional<Group> optionalGroup = timetableFacade.getGroup(renameForm.getRenameId());
        if (!optionalGroup.isPresent()) {
            log.error("Group with ID({}) no found", renameForm.getRenameId());
            redirectAttributes.addFlashAttribute("errorAlert",
                    "Attempt to rename group failed: group with ID(" + renameForm.getRenameId()
                            + ") could not be found. Please, resubmit the " + "form.");
            return "redirect:/timetable/management/university/groups";
        }
        Group group = optionalGroup.get();
        ...
        redirectAttributes.addFlashAttribute("successAlert",
                String.format("Group ID (%d) is now called %s", group.getId(), group.getName()));
    ...
    @GetMapping("/remove")
    public String removeGroup(RedirectAttributes redirectAttributes, @RequestParam("id") @IdValid("Group") long id) {

        Optional<Group> optionalGroup = timetableFacade.getGroup(id);
        if (!optionalGroup.isPresent()) {
            log.error("Group with ID({}) no found", id);
            redirectAttributes.addFlashAttribute("errorAlert",
                    "Attempt to remove group failed: group with ID(" + id + ") could not be found. Please, "
        ...
        redirectAttributes.addFlashAttribute("successAlert", "Group ID (" + id + ") was deleted");
    ...
    @PostMapping("/new")
    public String addNewGroup(RedirectAttributes redirectAttributes, @ModelAttribute @Valid NewItemForm newItemForm) {

        Group group = new Group(newItemForm.getName());
        group = timetableFacade.saveGroup(group);

        redirectAttributes.addFlashAttribute("successAlert",
                String.format("Group %s with ID (%d) added to the list", group.getName(), group.getId()));
        redirectAttributes.addFlashAttribute("editedId", group.getId());
