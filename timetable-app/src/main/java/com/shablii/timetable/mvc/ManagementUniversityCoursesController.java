...
import com.shablii.timetable.service.utility.predicates.SchedulePredicateCourseId;
...
public class ManagementUniversityCoursesController {
    ...
    @GetMapping("/")
    public String courses(Model model, @ModelAttribute("successAlert") String successAlert,
    ...
    public String courseSchedule(Model model, @ModelAttribute("scheduleForm") @Valid ScheduleForm scheduleForm,
            RedirectAttributes redirectAttributes) {

        Optional<Course> optionalCourse = timetableFacade.getCourse(scheduleForm.getId());
        if (!optionalCourse.isPresent()) {
            log.error("Course with ID({}) no found", scheduleForm.getId());
            redirectAttributes.addFlashAttribute("errorAlert",
                    "Attempt to locate schedule failed: course with ID(" + scheduleForm.getId()
        ...
        switch (scheduleForm.getScheduleOption()) {
            ...
            case DAY:
                DaySchedule daySchedule = scheduleFormatter.prepareDaySchedule(
                        new SchedulePredicateCourseId(scheduleForm.getId()), date, scheduleForm.isFiltered());
    ...
    public String rename(RedirectAttributes redirectAttributes,
        ...
        Optional<Course> optionalCourse = timetableFacade.getCourse(renameForm.getRenameId());
        if (!optionalCourse.isPresent()) {
            log.error("Course with ID({}) no found", renameForm.getRenameId());
            redirectAttributes.addFlashAttribute("errorAlert",
                    "Attempt to rename course failed: course with ID(" + renameForm.getRenameId()
                            + ") could not be found. Please, resubmit the " + "form.");
            return "redirect:/timetable/management/university/courses";
        }
        Course course = optionalCourse.get();
        ...
        redirectAttributes.addFlashAttribute("successAlert",
                String.format("Course ID (%d) is now called %s", course.getId(), course.getName()));
    ...
    @GetMapping("/remove")
    public String removeCourse(RedirectAttributes redirectAttributes, @RequestParam("id") @IdValid("Course") long id) {

        Optional<Course> optionalCourse = timetableFacade.getCourse(id);
        if (!optionalCourse.isPresent()) {
            log.error("Course with ID({}) no found", id);
            redirectAttributes.addFlashAttribute("errorAlert",
                    "Attempt to remove course failed: course with ID(" + id + ") could not be found. Please, "
        ...
        redirectAttributes.addFlashAttribute("successAlert", "Course ID (" + id + ") was deleted");
    ...
    public String addNewCourse(RedirectAttributes redirectAttributes,
        ...
        redirectAttributes.addFlashAttribute("successAlert",
                String.format("Course %s with ID (%d) added to the list", course.getName(), course.getId()));
        redirectAttributes.addFlashAttribute("editedId", course.getId());
