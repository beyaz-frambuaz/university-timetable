...
public class SemesterCalendar {
    ...
    public SemesterCalendar(@Value("${university.semester.start.date}") String startDate,
            @Value("${university.semester.end.date}") String endDate) {

        this.startDate = LocalDate.parse(startDate);
        this.endDate = LocalDate.parse(endDate);
        this.lengthInWeeks = (int) ChronoUnit.WEEKS.between(this.startDate, this.endDate) + 1;
    }

    public boolean getWeekParityOf(LocalDate date) {

        int dateWeekInYear = date.get(IsoFields.WEEK_OF_WEEK_BASED_YEAR);
        int weekInYearBeforeSemester = startDate.minusDays(1).get(IsoFields.WEEK_OF_WEEK_BASED_YEAR);
        return (dateWeekInYear - weekInYearBeforeSemester) % 2 == 0;
    }

    public boolean isSemesterDate(LocalDate date) {

        return !(date.getDayOfWeek() == DayOfWeek.SATURDAY || date.getDayOfWeek() == DayOfWeek.SUNDAY || date.isBefore(
    ...
    public String getSemesterDescription() {
        ...
        return startDate.format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM).withLocale(Locale.US)) + " - "
                + endDate.format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM).withLocale(Locale.US));
    }

    public String getDayDescription(LocalDate date) {

        return date.format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM).withLocale(Locale.US));
    }
    ...
    public String getWeekDescription(LocalDate date) {
        ...
        return monday.format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM).withLocale(Locale.US)) + " - "
                + friday.format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM).withLocale(Locale.US));
    }
    ...
    public String getMonthDescription(LocalDate date) {
        ...
        return firstOfMonth.format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM).withLocale(Locale.US)) + " - "
                + lastOfMonth.format(DateTimeFormatter.ofLocalizedDate(FormatStyle.MEDIUM).withLocale(Locale.US));
    }
    ...
    public String[] getDayShortDescription(LocalDate date) {
        ...
        return new String[] { date.getDayOfWeek().getDisplayName(TextStyle.SHORT, Locale.US).toUpperCase(),
                date.format(DateTimeFormatter.ofLocalizedDate(FormatStyle.SHORT).withLocale(Locale.US)) };
    }
